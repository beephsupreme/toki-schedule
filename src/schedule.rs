...
pub fn schedule() -> std::io::Result<()> {
    ...
    // Parse dates and save as json file
    let mut dates_vec: Vec<String> = Vec::new();
    for i in 0..number_of_dates {
        let dt = elements[i].replace("/", "-");
        dates_vec.push(dt);
    }

    let dates_json = serde_json::to_string(&dates_vec).unwrap();
    let mut file = File::create("dates.json").expect("Could not create file!");
    file.write_all(dates_json.as_bytes())
